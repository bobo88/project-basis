<template>
	<view class="u-page">
		<uni-nav-bar title="购买TODO" height="100rpx" leftIcon="arrow-left" color="#fff" backgroundColor="#1D1E1F" fixed
			:border="false" @clickLeft="returnBack"></uni-nav-bar>
		<view class="meal-tab">
			<view v-for="(item,index) in tabLists" :key="item" class="meal-tab-item" @click="onClickItem(index)">
				<text :class="current === index ? 'active' : ''">{{item}}</text>
				<view v-if="current === index" class="meal-tab-item-line"></view>
			</view>
		</view>
		...
		<view class="meal-num">
			<text class="txt2">购买数量</text>
			<view class="meal-number">
				<uni-icons @click="handleMinus" custom-prefix="iconfont" type="icon-jianhao"
					:color="stepperNum > 1 ? '#bbb' : '#666'" size="32rpx"></uni-icons>
				<input class="meal-number-input" type="number" @blur="onKeyInput" v-model="stepperNum" />
				<uni-icons @click="handleAdd" type="plusempty"
					:color="stepperNum < phoneRemainQuantity || stepperNum < 60 ? '#bbb' : '#666'" size="32rpx">
				</uni-icons>
			</view>
		</view>
		<view v-show="mealList.length && mealList[active].support" class="meal-coupon" @click="showCoupon()">
			...
			<view class="frc">
				<text v-if="couponObj.id"
					class="txt9">{{couponObj.couponType == 1 ? '-¥' : ''}}{{couponObj.couponType == 1 ? couponObj.couponValue : couponObj.deductionPrice}}{{couponObj.couponType == 2 ? '折' : ''}}</text>
			...
			<view class="popup-coupon-content">
				...
				<view v-if="cartList.length">
					<view class="coupon-view">
						<coupon v-for="(item, index) in cartList" :key="index" :select="couponObj.id===item.id"
							:dataObj="item" @setCouponObj="setCouponObj"></coupon>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 选择优惠券
			setCouponObj(id) {
				this.couponObj = this.couponObj.id === id ? {
					id: 0
				} : this.cartList.filter(item => item.id === id)[0]
				this.stepperNum = this.couponObj.id && this.couponObj.batchBuy ? this.stepperNum : 1
				this.countPrice()
				this.useCoupon()
			},
			useCoupon() {
				if (this.couponObj.couponType === 1) {
					this.xbPrice = this.xbPrice - Number(this.couponObj.couponValue) > 0 ? parseFloat((this.xbPrice -
						Number(this.couponObj.couponValue)).toFixed(2)) : 0.01
				} else if (this.couponObj.couponType === 2) {
					this.xbPrice = parseFloat((this.xbPrice * this.couponObj.deductionPrice / 10).toFixed(2)) <= 0 ? 0.01 :
						parseFloat((this.xbPrice * this.couponObj.deductionPrice / 10).toFixed(2))
				}
			},
			cancelCoupon() {
				this.couponObj = {
			...
			getMealInfo() {
				uni.$u.http.get('/api/pay/v2/meal/info', {
					data: {
						"phoneType": buyType[this.current],
					}
				}).then(res => {
			...
			countPrice() {
				if (this.mealList[this.active].remainQuantity === -1) {
					this.xbPrice = this.payActive === 0 ? parseFloat((this.mealList[this.active].actualPrice * 10 * this
						.stepperNum).toFixed(2)) : parseFloat((this.mealList[this.active].actualPrice * this
						.stepperNum).toFixed(2))
				} else {
					if (this.payActive === 0) {
						if (this.stepperNum > this.mealList[this.active].remainQuantity) {
							this.xbPrice = this.mealList[this.active].originalPrice === '' ? parseFloat((this.mealList[this
									.active].actualPrice * 10 * this.stepperNum).toFixed(2)) :
								parseFloat((((this.stepperNum - this.mealList[this.active].remainQuantity) * this.mealList[
									this.active].originalPrice) * 10 + (this.mealList[this.active].remainQuantity *
									this.mealList[this.active].actualPrice) * 10).toFixed(2))
						} else {
							parseFloat((this.xbPrice = this.mealList[this.active].actualPrice * 10 * this.stepperNum)
								.toFixed(2))
						}
					} else { // remainQuantity == 0 时的微信支付
						if (this.stepperNum > this.mealList[this.active].remainQuantity) {
							this.xbPrice = this.mealList[this.active].originalPrice === '' ?
								parseFloat((this.mealList[this.active].actualPrice * this.stepperNum).toFixed(2)) :
								parseFloat((((this.stepperNum - this.mealList[this.active].remainQuantity) * this.mealList[
									this.active].originalPrice) + (this.mealList[this.active].remainQuantity * this
									.mealList[this.active].actualPrice)).toFixed(2))
						} else {
							this.xbPrice = parseFloat((this.mealList[this.active].actualPrice * this.stepperNum).toFixed(
								2))
						}
					}
				}
			...
			sureBug() {
				...
				uni.$u.http.post('/api/pay/v1/order/create', taocan, {
					...
					if (this.payActive === 1) {
						...
						uni.$u.http.post('/api/pay/v2/starCoins/spend', {
							myOrderNum: myOrderNum
						}, {
							header: {
								sign: this.jiaqian(JSON.stringify({
									myOrderNum: myOrderNum
								}))
							}
						}).then(res => {
							uni.navigateTo({
								url: '/pages/order/status?active=success&starcoin=' + this.xbPrice
							});
						})
					}
					uni.hideLoading();
			...
			onKeyInput(event) {
				if (this.couponObj.id && !this.couponObj.batchBuy) {
					uni.showToast({
						title: '该优惠券不支持批量购买',
						duration: 1000,
						icon: 'none'
				...
				if (Number(event.target.value) < 1) {
					...
					this.$refs.popup_limit.open()
					this.stepperNum = 60
				} else if (Number(event.target.value) > this.phoneRemainQuantity) {
					this.stepperNum = this.phoneRemainQuantity
				} else {
					this.stepperNum = Number(event.target.value)
				}
				this.countPrice()
				if (this.payActive && this.mealList[this.active].support && this.mealList[this.active].renewalType === 1) {
					this.getUserCouponSort()
			...
			handleAdd() {
				...
				if (this.stepperNum === 60) {
					this.$refs.popup_limit.open()
				}
				if (this.stepperNum === this.phoneRemainQuantity || this.stepperNum === 60) {
					return
				}
				this.stepperNum += 1
				this.countPrice()
			...
			selectCoupon() {
				const discount = this.stepperNum > 1 ? this.cartList.filter(item => item.couponType === 2 && item.batchBuy).sort((a, b) => {
					return Number(a.deductionPrice) - Number(b.deductionPrice)
				}) : this.cartList.filter(item => item.couponType === 2).sort((a, b) => {
					return Number(a.deductionPrice) - Number(b.deductionPrice)
				})
				const deduction = this.stepperNum > 1 ? this.cartList.filter(item => item.couponType === 1 && item.batchBuy).sort((a, b) => {
					return Number(b.couponValue) - Number(a.couponValue)
				}) : this.cartList.filter(item => item.couponType === 1).sort((a, b) => {
					return Number(b.couponValue) - Number(a.couponValue)
				})
				...
				this.couponObj = Number(discount[0].deductionPrice) * this.xbPrice > this.xbPrice - Number(deduction[0]
					.couponValue) ? deduction[0] : discount[0]
				this.useCoupon()
			}
		}
	}
