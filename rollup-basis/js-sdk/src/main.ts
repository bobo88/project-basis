...
export const CLOUD_GAME_SDK: CloudGameSdk = {
    ...
    sdkInit: (options) => {
		...
		// 默认进来就触发一次【手机重力感应】判断
		CLOUD_GAME_SDK.windowOnresizeInit();
		if (options.onresize) {
			// 开启【手机重力感应】判断
			CLOUD_GAME_SDK.windowOnresize();
		}

    },
	windowOnresizeInit: () => {
		// 页面一进来就判断存储 【音视频盒子】的宽高
		SDK_CONFIG.boxW = SDK_CONFIG.myContainer.style.width;
		SDK_CONFIG.boxH = SDK_CONFIG.myContainer.style.height;
		

		let myContainerW = SDK_CONFIG.myContainer.style.width;
		let myContainerH = SDK_CONFIG.myContainer.style.height;
		let myVideoW = SDK_CONFIG.myVideo.style.width || SDK_CONFIG.myVideo.getAttribute('width');
		let myVideoH = SDK_CONFIG.myVideo.style.height || SDK_CONFIG.myVideo.getAttribute('height');
		
		CLOUD_GAME_SDK.resizeFun({myContainerW, myContainerH, myVideoW, myVideoH});
	},
	resizeFun: () => {
		let Height = document.body.offsetHeight || window.innerHeight || document.body.clientHeight || document.body.scrollHeight;
		let Width = document.body.offsetWidth || window.innerWidth;
		console.info('=== 开启【手机重力感应】判断(宽/高): ', Width, Height);
		if (Width > Height) {
			console.info('=== 手机横屏')
			SDK_CONFIG.phoneDirection = 1;

			SDK_CONFIG.myContainer.style.width = SDK_CONFIG.boxH;
			SDK_CONFIG.myContainer.style.height = SDK_CONFIG.boxW;

			SDK_CONFIG.myVideo.style.width = SDK_CONFIG.boxH;
			SDK_CONFIG.myVideo.style.height = SDK_CONFIG.boxW;
			SDK_CONFIG.myVideo.style.top = 0;
			SDK_CONFIG.myVideo.style.transform = 'rotate(0deg)';
		} else {
			console.info('=== 手机正常')
			SDK_CONFIG.phoneDirection = 0;

			SDK_CONFIG.myContainer.style.width = SDK_CONFIG.boxW;
			SDK_CONFIG.myContainer.style.height = SDK_CONFIG.boxH;
			SDK_CONFIG.myVideo.style.width = SDK_CONFIG.boxH;
			SDK_CONFIG.myVideo.style.height = SDK_CONFIG.boxW;
			SDK_CONFIG.myVideo.style.top = `-${SDK_CONFIG.boxW}`;
			SDK_CONFIG.myVideo.style.transform = 'rotate(90deg)';
		}
	...
	setVideoSize: () => {
		...
		// // ========================== ★★★★★ VIP Ended =========================================
		// if (playerVideo) {
		// 	playerVideo.setAttribute('width',  ((SDK_CONFIG.isLandscape || SDK_CONFIG.phoneDirection) ? ch : cw) + 'px')
		// 	playerVideo.setAttribute('height',  ((SDK_CONFIG.isLandscape || SDK_CONFIG.phoneDirection) ? cw : ch) + 'px')

		// 	// playerVideo.style.width =((SDK_CONFIG.isLandscape || SDK_CONFIG.phoneDirection) ? ch : cw) + 'px';
		// 	// playerVideo.style.height = ((SDK_CONFIG.isLandscape || SDK_CONFIG.phoneDirection) ? cw : ch) + 'px';
		// }
	},
	/**
	 * 
	...
	touchClick: (event, type, bool) => {
		...
		var canvasWidth = parseInt(SDK_CONFIG.boxW);
		var canvasHeight = parseInt(SDK_CONFIG.boxH);

		SDK_CONFIG.boxOffsetTop = SDK_CONFIG.myContainer.offsetTop;
		SDK_CONFIG.boxOffsetLeft = SDK_CONFIG.myContainer.offsetLeft;
		console.log('========== Box: ', SDK_CONFIG.boxW, SDK_CONFIG.boxH, SDK_CONFIG.boxOffsetTop, SDK_CONFIG.boxOffsetLeft);
		...
		for (var i = 0; i < touches.length; i++) {
			...
			console.log(555555, SDK_CONFIG.videoWidth, SDK_CONFIG.videoHeight, canvasWidth, canvasHeight) // 555555 1920 1080 250 600
			// =============================  SDK_CONFIG.boxOffsetTop, SDK_CONFIG.boxOffsetLeft 
			// 横屏游戏
			if (SDK_CONFIG.isLandscape) {
				// 根据游戏的分辨率 设置
				posX = (SDK_CONFIG.videoWidth * 1.0 * (touches[i].clientY - SDK_CONFIG.boxOffsetTop)) / canvasHeight
				posY = (SDK_CONFIG.videoHeight * 1.0 * (canvasWidth - touches[i].clientX + SDK_CONFIG.boxOffsetLeft)) / canvasWidth
				console.log('我是client坐标：', touches[i].clientX, touches[i].clientY)
				// 重力感应
				if (SDK_CONFIG.phoneDirection) {
					// 根据游戏的分辨率 设置
					posX = ((touches[i].clientX - SDK_CONFIG.boxOffsetLeft) * SDK_CONFIG.videoWidth * 1.0) / canvasHeight
					posY = ((touches[i].clientY - SDK_CONFIG.boxOffsetTop) * SDK_CONFIG.videoHeight * 1.0) / canvasWidth
					console.log('换算后：', posX, posY)
				}
				...
				// 根据游戏的分辨率 设置
				posX = ((touches[i].clientX - SDK_CONFIG.boxOffsetLeft) * SDK_CONFIG.videoWidth * 1.0) / canvasWidth
				posY = ((touches[i].clientY - SDK_CONFIG.boxOffsetTop) * SDK_CONFIG.videoHeight * 1.0) / canvasHeight
				// 重力感应
				if (SDK_CONFIG.phoneDirection) {
					// 根据游戏的分辨率 设置
					console.log(555555, SDK_CONFIG.videoWidth, SDK_CONFIG.videoHeight, canvasWidth, canvasHeight) // 555555 1920 1080 250 600

					posX = ((canvasWidth - touches[i].clientY + SDK_CONFIG.boxOffsetTop) * SDK_CONFIG.videoWidth * 1.0) / canvasWidth
					posY = ((touches[i].clientX - SDK_CONFIG.boxOffsetLeft) * SDK_CONFIG.videoHeight * 1.0) / canvasHeight
				}
				let buffer = onClickHandle[type](
					posX.toFixed(2),
					posY.toFixed(2),
