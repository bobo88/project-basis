...
{	
	...
	for(i = 0;i < chanelBuffer.length;i++) {
...
function checkMultiLoginInfo(input)
{
 ...
 // 设置分辨率
 function ExexuteSetPhoneSize(width,height) {
	console.log('== 进来了 设置分辨率')
    // var jsonObj = {"data":{"keyCode":code}, "event":"keyCode"};
    var jsonObj = {
        data: {
            width:width,
...
function ExexuteSendBitRate(data) {
    // var jsonObj = {"data":{"keyCode":code}, "event":"keyCode"};
    var jsonObj = {
        "type"  : 2,
		"data"  : {
			"definition": 1,             //值为1到5 个等级
...
{
	 var sn = "RK3923C1201900139";
	 var jsonObj = {"type":2,"data":{"definition":level}};	 
	 jsonObj = {"type":2,"data":{"definition":level, "clientType":"h5", "sceneType":"cloudGame"}};	 
	 var jsonStr = JSON.stringify(jsonObj);
	 var outPut = new TextEncoder("utf-8").encode(jsonStr);
 ...
 function fpsSet(frameRate)
{
	var sn = "RK3923C1201900139";
    var jsonObj = { "type": 1, "data": { "frame_rate": frameRate } };
    var jsonStr = JSON.stringify(jsonObj);
