  ...
  methods: {
    ...
    getVideoProductData() {
      getVideoProduct().then(response => {
        ...
        const tempDateArr = [
          ...this.crawlData.map(i => i.dt),
          ...this.duetData.map(i => i.dt),
          ...this.localData.map(i => i.dt),
          ...this.mvData.map(i => i.dt),
          ...this.normalData.map(i => i.dt)
        ]
        const sigleDateArr = [...new Set(tempDateArr)].sort()
        this.datetimeArr = sigleDateArr.map(i => {
          return this.$filters.dateFormatMmDd(i)
        }) || []
